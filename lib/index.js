		...
		scope.effect(() => scope.connection.fetch.register({
			...
			fetch: async (request) => {
				...
				if (request.method === "GET") {
					...
					const child = scope.sessions.get(childSessionId);
					const live = scope.agents.get(childSessionId) !== void 0;
					const allowed = allowedFor(parentSessionId);
					return Response.json({
						ok: true,
						live,
						current: currentRouteOf(child?.requestHeader()?.config) ?? null,
						allowed: allowed === void 0 ? null : allowed
					});
				}
				let body;
				try {
				...
				const child = scope.sessions.get(childSessionId);
				if (child === void 0) return failure(404, "session-not-found", `no session "${childSessionId}"`);
				if (child.header.origin !== "subagent" || String(child.header.parentSession ?? "") !== parentSessionId) return failure(403, "not-a-direct-child", `session "${childSessionId}" is not a direct child of "${parentSessionId}"`);
				if (scope.agents.get(childSessionId) === void 0) return failure(409, "child-not-live", `session "${childSessionId}" has no live agent; retargeting a cold child is refused`);
				const allowed = allowedFor(parentSessionId);
