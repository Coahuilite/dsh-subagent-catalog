...
**谁来让它「下次活动」——队列不会、也不能自己唤醒子代理。** 这一点在实测中被踩到过一次，记清楚：

- 覆盖项只在那个 agent **真的再次发请求**时才生效，`agent/status` 也只在那时触发；本插件从不主动唤醒（这正是它不越上游那条线的原因）。
- **Agent Teams 的 `send_message` 唤不动已结算的成员**：它只解析**活跃**名册，对已结算的子代理报 `active teammate "<id>" not found`（`TEAM_MEMBER_NOT_FOUND`）。想靠它来"让改动生效"是不通的。
- 官方真正的续唤路径是**子代理自己的 continuation 通道**（`subagent.prompt` Remote，父会话用它继续一个可继续的子代理）。所以排队生效的时机是：**父会话下次照常派活给它**。
- 另外：**子代理无法自证自己的思考档位**（agent 从会话内部读不到生效值，只会回一句"无法确认"）。要核对档位，看面板读的 `lastUsed.reasoningEffort` —— 它由真实的 `request/header` 写入，是唯一可信来源。
